...
def read_rows():
    if not RESULTS_FILE.exists():
        raise FileNotFoundError(f"Missing {RESULTS_FILE}. Run exp/mlrunner.py first.")
...
def write_figure(rows):
    ...
    content = rf"""% Auto-generated by exp/generate_fixed_T_tikz.py.
...
"""
    ...
    ucb_content = rf"""% Auto-generated by exp/generate_fixed_T_tikz.py.
% Source file: {RESULTS_FILE}
\begin{{figure*}}[t]
\centering
{ucb_cost_panel}
...
\par\vspace{{0.15em}}
{combined_legend()}
\caption{{MovieLens fixed-horizon UCB experiments over ten repeats. Panel (a) reports native attack costs for our two offline constructions and the bounded heuristic of \citet{{jun2018adversarial}}; the dashed curve is a scaled $S_T=T^{{2/3}}(K\log T)^{{1/3}}$ bound chosen to upper-bound the displayed mean-plus-standard-deviation envelope of our costs. Panel (b) reports total target-arm exposure $N_K/T$ and, for our attacks, post-deployment success $N_K^{{\on}}/H$. \citet{{xu2021observation}} is omitted because its observation-free cost dominates the displayed scale.}}
\label{{fig:fixed_T_sweep}}
\end{{figure*}}
"""
    ts_content = rf"""% Auto-generated by exp/generate_fixed_T_tikz.py.
% Source file: {RESULTS_FILE}
\begin{{figure*}}[t]
\centering
{ts_cost_panel}
...
\par\vspace{{0.15em}}
{combined_legend()}
\caption{{MovieLens fixed-horizon Thompson Sampling experiments over ten repeats. Panel (a) reports native attack costs for our two offline constructions and the bounded heuristic of \citet{{zuo2024near}}; the dashed curve is a scaled $S_T=T^{{2/3}}(K\log T)^{{1/3}}$ bound chosen to upper-bound the displayed mean-plus-standard-deviation envelope of our costs. Panel (b) reports total target-arm exposure $N_K/T$ and, for our attacks, post-deployment success $N_K^{{\on}}/H$. \citet{{xu2021observation}} is omitted because its observation-free cost dominates the displayed scale.}}
